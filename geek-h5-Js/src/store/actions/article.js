...
const setInfo = partialInfo => ({ type: 'article/set_info', payload: partialInfo })
...
const setComment = partial =>    ({ type: 'article/set_comment',  payload: partial })
...
const getCommentList = ({ type, source }) => {
  return async dispatch => {
    try {
      ...
      // console.log('请求参数', params) // {type: 'a', source: '8123'}
      const res = await http.get('/comments', {   params    })
